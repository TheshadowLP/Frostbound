...
public class WorldSettingConfig {
    ...
    static
    {
        ...
        biomeOffsets = BUILDER
                ...
                .defineList("Biome Temperature Offsets", List.of(),
                        it ->
                        {
                            ...
                            {
                                if (list.size() == 2)
                                {   Frostbound.LOGGER.warn("Falling back to legacy code for config setting {} in \"Biome Temperature Offsets\". Please update to the new standard!", list.get(0));
                                }
        ...
        biomeTemps = BUILDER
                ...
                .defineList("Biome Temperatures", net.shadowbeast.frostbound.util.nbt.ListBuilder.begin(
                        ...
                        it ->
                        {
                            ...
                            {
                                if (list.size() == 2)
                                {   Frostbound.LOGGER.warn("Falling back to legacy code for config setting {} in \"Biome Temperatures\". Please update to the new standard!", list.get(0));
                                }
                                return list.get(0) instanceof String && list.get(1) instanceof Number && (list.size() < 3 || list.get(2) instanceof Number) && (list.size() < 4 || list.get(3) instanceof String);
                            }
                            return false;
    ...
    public static void setup()
    {
        ...
        ModLoadingContext.get().registerConfig(ModConfig.Type.COMMON, SPEC, "frostbound/world_settings.toml");
    }
